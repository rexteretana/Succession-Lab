...
export function ConditionExplorer({ conditions }: { conditions: TestamentaryCondition[] }) {
  ...
  return <section className={`axes-explorer ${expanded ? 'axes-expanded' : ''}`} aria-label="Condition space">
    ...
    <div className="axes-meta"><strong>{plottedCount} plotted conditions</strong><span>{unplotted.length} substitution-only {unplotted.length === 1 ? 'clause' : 'clauses'}</span><span>{groups.length} distinct positions</span></div>
    ...
    <div className="axes-selector" aria-label="Choose a clause">{conditions.map((c,i) => { const plotted = Boolean(conditionCoordinate(c)); return <button className={plotted ? 'is-plotted' : 'is-outside'} key={c.id} aria-label={`${plotted ? 'Condition' : 'Substitution-only clause'} ${i+1}`} aria-pressed={i===selected} onClick={() => select(i)} title={c.text}>{i+1}</button>; })}</div>
    <div className="axes-selection" aria-live="polite"><strong>{coordinate ? 'Condition' : 'Substitution-only clause'} {selected+1}</strong>{coordinate ? <div className="axes-tags"><span>{axes.conduct}</span><span>{axes.control}</span><span>{axes.effect}</span></div> : <div className="axes-tags axes-tags-na"><span>Not applicable to the three-axis test</span></div>}<p>{current.text}</p>{!coordinate && <p className="axes-warning"><strong>Correctly outside the cube.</strong> This clause activates a substitution but states no separate positive or negative condition, source of control, or suspensive/resolutory effect.</p>}<button onClick={() => { if(expanded) close(); document.getElementById(`condition-${current.id}`)?.scrollIntoView({block:'start',behavior:'smooth'}); }}>Read linked dispositions →</button></div>
